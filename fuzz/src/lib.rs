...
impl bevy_mikktspace::Geometry for TriangulatedGeometry {
...
impl Geometry {
    pub fn validate(&mut self) -> Result<(), arbitrary::Error> {
        ...
        for face in faces.iter_mut() {
            let indices = core::mem::take(&mut face.vertex_indices);
            face.vertex_indices = indices.into_iter().map(|i| i % vertices.len()).collect();
        }
        ...
        // Known failure: faces must be tris or quads
        for face in faces.iter() {
            if !(3..=4).contains(&face.vertex_indices.len()) {
                return Err(arbitrary::Error::IncorrectFormat);
            }
        }
...
impl bevy_mikktspace::Geometry for Geometry {
    ...
    fn position(&self, face: usize, vert: usize) -> [f32; 3] {
        self.vertices[self.faces[face].vertex_indices[vert]].position
    }

    fn normal(&self, face: usize, vert: usize) -> [f32; 3] {
        self.vertices[self.faces[face].vertex_indices[vert]].normal
    }

    fn tex_coord(&self, face: usize, vert: usize) -> [f32; 2] {
        self.vertices[self.faces[face].vertex_indices[vert]].tex_coord
    }
    ...
    fn set_tangent(
        ...
        self.vertices[self.faces[face].vertex_indices[vert]].tangent = tangent;
    }
}
